...
###### 好处:
1. 在 entry.s 中，boot_page_table_sv39 采用了一种大页映射方式。这种方式的优势在于能够将操作系统从物理地址模式切换到虚拟地址模式时，省去多级映射关系的处理，从而简化了页表层次结构。同时，这种映射方式能够更高效地查找和缓存页表项，大幅提升内存访问性能。
2. 减少了所需页表项的数量，减小了内存管理开销。由于页表项数量减少，"一个大页"的页表映射方式可以提高访问效率，在分级页表中，访问一个虚拟页需要多次查找页表项，而使用"一个大页"的页表映射方式只需要一次查找即可。
3. 减少TLB miss，当一个大页被加载到TLB中，可以覆盖更多的虚拟地址范围，减少了TLB缺失的次数，提高了内存访问的效率。
4. 对于需要大量连续内存的应用程序，一个大页的映射方式能很好地满足其内存需要与性能要求。

###### 坏处:
1. 使用大页进行映射时，每个页表项对应的地址范围较大（在 Sv39 下为 1GiB）。当发生缺页异常或需要页面置换时，需要将整个大页的内容（1GiB）全部交换到硬盘，或者从硬盘换回。这会导致：
- 性能下降：大量数据的交换会显著增加 I/O 开销，降低程序的运行速度。
- 换入换出同步性问题：换回时，所有内容必须整体写回内存，进一步增加了系统开销。
- 局部性问题：进程可能只使用大页的一小部分内容，但仍需要处理整个大页，浪费了存储和时间资源。
2. 内存利用率低与内存碎片问题
- 高内存需求：使用大页表时，每个页的映射覆盖范围较大，内存需求会急剧增加。特别是对于多进程环境，即使每个进程只需要很小的内存，也必须分配整个大页的空间，造成资源浪费。
- 内存碎片化：大页的分配需要连续的物理内存，容易导致内存碎片化问题。
当系统内存碎片化严重时，可能无法为大页分配足够的连续空间，进一步降低内存利用率。
3.  大页表内存开销大
- 页表空间占用多：在大页映射模式下，页表项数量减少，但每个页表项覆盖的地址范围更大。由于大页通常需要更高一级的页表，页表本身的占用空间可能更大。例如，在 RISC-V 的 Sv39 模式下，大页映射会减少多级页表的使用，但每一级页表的粒度变大，容易浪费内存。
- 进程切换频繁时的负面影响：进程切换需要切换虚拟地址空间，而大页映射会导致频繁的内存页换入换出，影响性能。

#### 扩展练习 Challenge：实现不考虑实现开销和效率的LRU页替换算法（需要编程）
challenge部分不是必做部分，不过在正确最后会酌情加分。需写出有详细的设计、分析和测试的实验报告。完成出色的可获得适当加分。

在 LRU 算法中，目标是寻找最近最少使用的页面，以便在需要页面置换时优先淘汰这些页面。实现最精确的 LRU 算法需要实时监控每个内存页的访问情况，并将被访问的页面动态移动到链表的前端。然而，在本次实验中，由于缺乏相应的硬件支持，我们无法准确获取页面的访问时间信息，只有在发生 Page Fault 异常时，才能确认某些页面被访问。但如果仅依赖于 Page Fault 来调整链表结构，其效果接近于 FIFO，难以体现 LRU 算法的优势。

为尽可能接近 LRU 算法的效果，不考虑额外的开销和效率问题，我们引入了时钟中断机制。

通过时钟中断，我们可以借助页面表项（PTE）中的 PTE_A 位（Accessed 位）来判断页面的访问情况。在内存页被访问时，硬件会自动将该页对应的 PTE_A 位置为 1。我们通过在时钟中断期间检查这些 PTE_A 位的状态，识别哪些页面在两次时钟中断之间被访问过，并动态调整这些页面在置换链表中的位置。具体而言，将被访问的页面移动到链表的末尾，并清除其 PTE_A 位。理论上，随着时钟中断的频率增加，该实现会更接近于理想的 LRU 算法。

在代码实现上，其他部分的逻辑可以复用 FIFO 的代码，仅需要重写 tick_event 函数：

```c
static int
_lru_tick_event(struct mm_struct *mm)
{ 
    // 获取 LRU 链表的头节点，head 是循环链表
    list_entry_t* head = (list_entry_t*)mm->sm_priv;

    // 初始化当前节点指针 cur，初始指向链表头节点
    ...
    while (cur->next != head)
    {
        ...
        // 检查页表项中的 PTE_A 位，表示该页面是否被访问过
        if (*ptep & PTE_A)      
        {
            // 页面在一段时间内被访问过，将其从链表中移到末尾，表示最近使用
            list_entry_t* temp = cur->prev; // 暂存当前节点的前驱节点，用于恢复遍历位置
            list_del(cur);                  // 从链表中删除当前节点
            *ptep &= ~PTE_A;                // 清除页面的 PTE_A 位，表示访问标记被清除
            list_add(head, cur);            // 将当前节点添加到链表末尾
            cur = temp;                     // 恢复当前节点为前驱节点，继续遍历
        }
    }
...
_lru_tick_event 函数用于在每次时钟中断触发时，遍历 LRU 置换链表，通过检查每个页面对应的页表项（PTE）中的 PTE_A 位（访问标志位），判断页面是否在中断周期内被访问过。对于被访问过的页面（PTE_A 位为 1），函数会将其从链表当前位置移除，并重新添加到链表的末尾，表示它是最近被使用的页面，同时清除其 PTE_A 位，重置访问标志。
...
_lru_check_swap(void) {
    // 页面初始状态：d1 c1 b1 a1
    // 模拟时钟中断，触发 LRU 算法清除页面访问标记（PTE_A）
    swap_tick_event(check_mm_struct);
    // 页面状态更新为：a0 b0 c0 d0
    // 此时所有页面的访问标记被清除，链表顺序未变。

    // 模拟写入虚拟页面 e，触发缺页异常
    cprintf("write Virt Page e in lru_check_swap\n");
    *(unsigned char *)0x5000 = 0x0e;  // 写入虚拟地址 0x5000，表示页面 e
    assert(pgfault_num == 5);         // 验证缺页异常次数是否为 5
    // 页面状态更新为：e1 a0 b0 c0
    // 页面 e 替换掉最近最少使用的页面 d，访问标记 PTE_A 被设置。

    // 模拟访问页面 c，手动设置其访问标记
    cprintf("write Virt Page c and set Access bit\n");
    pte_t *ptep = get_pte(check_mm_struct->pgdir, 0x3000, 0);  // 获取页面 c 的 PTE
    *ptep |= PTE_A;  // 手动设置页面 c 的访问标记 PTE_A
    // 页面状态更新为：e1 a0 b0 c1

    // 模拟再次触发时钟中断，调整 LRU 链表顺序
    swap_tick_event(check_mm_struct);
    // 页面状态更新为：c0 e0 a0 b0
    // 页面 c 和 e 被移动到链表末尾，PTE_A 标记被清除。

    // 模拟写入页面 d，触发缺页异常
    cprintf("write Virt Page d in lru_check_swap\n");
    *(unsigned char *)0x4000 = 0x0e;  // 写入虚拟地址 0x4000，表示页面 d
    assert(pgfault_num == 6);         // 验证缺页异常次数是否为 6
    // 页面状态更新为：d1 c0 e0 a0
    // 页面 d 替换掉最近最少使用的页面 b，PTE_A 被设置。

    // 模拟写入页面 b，再次触发缺页异常
    cprintf("write Virt Page b in lru_check_swap\n");
    *(unsigned char *)0x2000 = 0x0e;  // 写入虚拟地址 0x2000，表示页面 b
    assert(pgfault_num == 7);         // 验证缺页异常次数是否为 7
    // 页面状态更新为：b1 d0 c0 e0
    // 页面 b 替换掉最近最少使用的页面 a，PTE_A 被设置。
